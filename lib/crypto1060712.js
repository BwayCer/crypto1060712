...
function crypto1060712( md5 ) {
    ...
    this.decrypt = function ( txtCode, strPassword ) {
        ...
        ansCode = _encryptB( -1, ansCode, keyCode[ 2 ] );
        ansCode = _encryptA( _encryptA_toTxtCodeList( ansCode ), keyCode[ 1 ] );
    ...
    /**
     * 第一階段編碼用文本代碼清單： 提供
     * {@link module:crypto1060712.decrypt|crypto1060712.decrypt}
     * 使用。
     *
     * @memberof module:crypto1060712~
     * @func _encryptA_toTxtCodeList
     * @param {String} txtcode - 文本代碼。
     * @return {Array} 文本代碼清單。
     */
    function _encryptA_toTxtCodeList( txtCode ) {
        var val;
        var idx = 0;
        var len = txtCode.length;
        var txtCodeList = [];
        ...
        while ( idx < len ) {
            ...
            txtCodeList.push( parseInt( val, 16 ) );
        }
